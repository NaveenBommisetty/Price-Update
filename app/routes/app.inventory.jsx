...
export const loader = async ({ request }) => {
  ...
  // Normalize plan name
  const planKey = (subscription?.name || "free").toLowerCase();

  // ✅ Plan → inventory selection limit
  let selectionLimit = 50; // Free default
  if (planKey.includes("plus")) selectionLimit = 100;
  if (planKey.includes("pro")) selectionLimit = null; // Unlimited

  return json({
    subscription,
    planName: subscription?.name || "Free",
    selectionLimit, // number | null (null = unlimited)
...
export default function InventoryPage() {
  // ✅ plan info
  const { subscription, planName, selectionLimit } = useLoaderData();
  const isPaid = Boolean(subscription);

  // ✅ derived flags
  const isUnlimited = selectionLimit == null; // Pro
  const LIMIT_LABEL = isUnlimited ? "Unlimited" : String(selectionLimit);
  ...
  useEffect(() => {
    ...
    for (const p of products) {
      ...
      for (const v of p?.variants?.nodes || []) {
        const rowId = v?.inventoryItem?.id || v?.id;
  ...
  /* =========================
     ✅ LIMIT HELPERS (Free/Plus capped, Pro unlimited)
  ========================= */
  const capSelection = useCallback(
    (ids) => {
      ...
      setTone("warning");
      setMsg(`Your ${planName} plan allows selecting up to ${selectionLimit} items.`);
  ...
  // ✅ Polaris IndexTable selection handler
  const handleSelectionChange = useCallback(
    (selectionType, toggleType, selection, position) => {
      ...
      if ((selectionType === "all" || selectionType === "page") && toggleType === true) {
        // Pro => allow selecting all visible rows
        if (isUnlimited) {
          setSelectedResources(visibleRows.map((r) => r.id));
          setAllResourcesSelected(false);
          return;
        }

        // Free/Plus => cap to first N
        const firstIds = visibleRows.map((r) => r.id).slice(0, selectionLimit);
        setSelectedResources(firstIds);
        setAllResourcesSelected(false);

        if (visibleRows.length > selectionLimit) {
          setTone("warning");
          setMsg(`Your ${planName} plan allows selecting up to ${selectionLimit} products. Upgrade to Plus/Pro for more.`);
        }
      ...
      if (selectionType === "range") {
        ...
        if (Array.isArray(selection) && selection.length === 2) {
          ...
          start = selection.start ?? selection[0];
          end = selection.end ?? selection[1];
        }
        ...
        if (Number.isFinite(start) && Number.isFinite(end)) {
          ...
          setSelectedResources((prev) => {
            if (toggleType) {
              return capSelection([...prev, ...rangeIds]);
            }
    ...
    [visibleRows, isUnlimited, selectionLimit, capSelection, planName]
  );

  const selectedItemsCount = allResourcesSelected ? "All" : selectedResources.length;

  // ✅ used for row disable (only for Free/Plus)
  const limitReached = !isUnlimited && selectedResources.length >= selectionLimit;
  ...
  const saveAll = useCallback(async () => {
    try {
      ...
      // Free/Plus only: enforce cap on bulk save too
      if (!isUnlimited && targetIds.length > selectionLimit) {
        setTone("warning");
        setMsg(`Please select up to ${selectionLimit} rows for bulk save (${planName} limit).`);
        return;
      }
  ...
  }, [edited, locationId, rows, selectedResources, clearSelection, isUnlimited, selectionLimit, planName]);
    ...
    >
      {msg ? (
        <Box paddingBlockEnd="300">
          <Banner
            title={tone === "success" ? "Update" : tone === "warning" ? "Limit" : "Error"}
            tone={tone}
          >
            <p>{msg}</p>
          </Banner>
                ...
                <InlineStack align="space-between" blockAlign="center">
                  <InlineStack gap="200" blockAlign="center">
                    ...
                    {selectedItemsCount !== 0 ? (
                      <Badge tone="info">
                        {selectedItemsCount} selected {isUnlimited ? "" : `(limit ${selectionLimit})`}
                      </Badge>
                    ) : null}
              ...
              <Box paddingBlockStart="300">
                ...
                >
                  {visibleRows.map((r, idx) => {
                    ...
                    const rowDisabled = limitReached && !isSelected; // ✅ only disables for Free/Plus
